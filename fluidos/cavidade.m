function [u,v,p,x,y]=cavidade(mu,rho)
% fluido na cavidade
% https://youtu.be/iwL8ashXhWU?t=2294
% entrada:
%   mu=viscosidade
%   rho=densidade
% saida:
...
for it=1:nt-1
  ...
  nit=50;
	...
	%while 1
		...
		% calcula na fronteira usando as condicoes de fronteira (derivadas parciais)
		p(1,:)=p(2,:);
		p(nx,:)=p(nx-1,:);
		p(:,1)=p(:,2);
	endfor
	...
	for i=2:nx-1
		for j=2:ny-1
			u(i,j)=un(i,j) -un(i,j)*dt/dx*(un(i,j)-un(i-1,j)) -vn(i,j)*dt/dy*(un(i,j)-un(i,j-1)) -(dt/(2*rho*dx))*(p(i+1,j)-p(i-1,j)) + nu*(dt/dx^2*(u(i+1,j)-2*u(i,j)+u(i-1,j))+dt/dy^2*(u(i,j+1)-2*u(i,j)+u(i,j-1)));
